...
def set_gspace(n_angle, flip=True, cutoff_dilation=False):
    ...
    #ReResNet Orientation: 8 Fix Params: False Cutoff_Dilations:0

    if flip: # dihedral group
        print('D%d group' % Orientation)#D8 group
    ...
    #返回gspace=D8, fixparams=flase, cutoff_dilation=0
...
if hasattr(model, 'gspace_orientation'):
    ...
    #参数 8 1 0,返回gspace=D8, fixparams=flase, cutoff_dilation=0
else:
    import joint_model
...
FIELD_TYPE = {
    "trivial": trivial_feature_type,#平凡特征类型
    "regular": regular_feature_type,#常规特征类型
}
...
class ReResNet(BaseBackbone):
    ...
    #strides=[1, 2, 1, 1], dilations=[1, 1, 2, 4], out_indices=out_indices
    def __init__(self,
                 depth,
                 in_channels=3,
                 stem_channels=64,
        ...
        self.block, stage_blocks = self.arch_settings[depth] #50: (Bottleneck, (3, 4, 6, 3)),
