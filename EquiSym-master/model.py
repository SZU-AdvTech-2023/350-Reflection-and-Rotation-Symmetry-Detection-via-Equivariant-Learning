...
class SymmetryDetectionNetwork(nn.Module):
    def __init__(self, args=None, img=None):
        super(SymmetryDetectionNetwork, self).__init__()
        self.args = args
        self.sync_bn = args.sync_bn  # True
        self.backbone_model = args.backbone  # resnet(str)
        self.output_stride = 8
        self.freeze_bn = False
        self.num_classes = 1
        self.n_angle = args.n_angle  # 8(int)
        self.angle_interval = 360 / self.n_angle
        self.eq_cnn = args.eq_cnn  # False
        self.get_theta = args.get_theta  # 0(int)
        ...
        print("self.eq_cnn: " + str(self.eq_cnn))
        ...
        if self.eq_cnn:
            ...
            self.backbone = ReResNet(depth=self.args.depth, strides=[1, 2, 1, 1],
                                     dilations=[1, 1, 2, 4], out_indices=out_indices)
            # print(self.backbone)


            # 主干网络，作用就是提取图片中的信息
            # depth=50,strides=[1, 2, 1, 1],dilations=[1, 1, 2, 4],out_indices = (3,)

            if args.load_eq_pretrained:  # 默认1(int)
                print('loading pretrained e2cnn backbone')
                # hardcode for now
                eq_ckpt = torch.load(args.eq_model_dir)['state_dict']
                # ./weights/re_resnet50_custom_d8_batch_512.pth
        ...
        if self.get_theta in [10]:
            ...
            if self.eq_cnn:
                ...
                self.decoder_theta = Decoder(n_angle + 1, self.backbone_model, BatchNorm, last_conv_only, theta_convin,
                                             last_convout, use_bn=True, restrict=False)
            else:
                self.decoder_theta = build_decoder(n_angle + 1, self.backbone_model, BatchNorm, last_conv_only,
                                                   theta_convin, last_convout, use_bn=True)

        if self.eq_cnn:
            from modeling.eq_module import ASPP
            self.aspp = ASPP(self.output_stride, in_channels=res_featdim[args.depth])
            # output_stride为8，res_featdim[args.depth]为2048（深度50）
        else:
            self.aspp = build_aspp(self.backbone_model, self.output_stride, BatchNorm,
                                   inchannels=res_featdim[args.depth])
        ...
        if self.eq_cnn:
            ...
            self.decoder_axis = build_decoder(self.num_classes, self.backbone_model, BatchNorm, last_conv_only,
                                              last_convin, last_convout, use_bn=True)
    ...
    def forward(self, img, lbl, mask, is_syn, a_lbl=None, sym_type='reflection', vis_only=False):
        ...
        # 编码器Enc由一个E(2)-等变的ResNet和一个相邻的空间金字塔池化（ASPP）模块组成。
        feat = self.backbone(img)
        feat = self.aspp(feat)

        if self.get_theta in [10]:
            ### [Step 2a]: Apply auxiliary DEC 辅助解码
        ...
        if self.get_theta:
            ...
            theta_loss = F.cross_entropy(theta_out, a_lbl, weight=weight, reduction='none') / weight.sum(dim=1,
                                                                                                         keepdim=True)
